...
% rms of resultant velocity, used for the automatic method. 
% 
% Input variables
...
function [newIMU, newQTM, minRMS, index, relevantDataset] = selectDataByMinRmsVel(timeframes, fs, isOverlap, IMU, QTM, idx_freq)
    ...
    for segment = 1:noOfRequiredSegments
        ...
        % higher than the actual velocities.
        minRMS(segment, 1) = 30;
        ...
        for dataset = 1:length(IMU.C1)
            ...
            booleanArray = ones(length(IMU.C1(dataset).Top(idx_freq).vel_x),1);
...
%             vel_x = booleanArray.*IMU.C1(dataset).Top(idx_freq).vel_x;
            

            % Loop over all elements in the dataset
            for i = 1:length(IMU.C1(dataset).Top(idx_freq).vel_x)-samples(segment)
                
                % Only include the useful parts => exclude zero parts
                % defined by boolean array
                if all(booleanArray(i:i+samples(segment)-1))
                    
                    % Compute resultant velocity for a single timeframe
                    velRes = sqrt(IMU.C1(dataset).Top(idx_freq).vel_x(i:(i+samples(segment)-1))+IMU.C1(dataset).Top(idx_freq).vel_z(i:(i+samples(segment)-1)));
                    
                    % Check if the new RMS is lower than the current rms
                    if rms(velRes) < minRMS(segment,1)
                        
                        % Redefine index, minimal RMS and relevant dataset
                        index(segment,  1)          = i;
                        minRMS(segment, 1)          = rms(velRes);
    ...
    if any(strcmp(fieldnames(IMU),'C2'))
        ...
        for segment = 1:noOfRequiredSegments
            ...
            for dataset = 1:length(IMU.C2)
                ...
                % Initialize boolean array with same dimension
                booleanArray = ones(length(IMU.C2(dataset).Top(idx_freq).vel_x),1);
                ...
                vel_x = booleanArray.*IMU.C2(dataset).Top(idx_freq).vel_x;
                vel_z = booleanArray.*IMU.C2(dataset).Top(idx_freq).vel_z;

                % Perform a check to see if the correct 
                assert(nnz(vel_x) == sum (booleanArray),'The number of nonzero elements do not match the Boolean array.')
                assert(nnz(vel_z) == sum (booleanArray),'The number of nonzero elements do not match the Boolean array.')
    
                % Loop over all elements in the dataset
                for i = 1:length(IMU.C2(dataset).Top(idx_freq).vel_x)-samples(segment)
                    
                    % Only include the useful parts => exclude zero parts
                    % defined by boolean array
                    if all(vel_x(i:i+samples(segment)-1))
                        
                        % Compute resultant velocity for a single timeframe
                        velRes = sqrt(IMU.C2(dataset).Top(idx_freq).vel_x(i:(i+samples(segment)-1))+IMU.C2(dataset).Top(idx_freq).vel_z(i:(i+samples(segment)-1)));
                        
                        % Check if the new RMS is lower than the current rms
                        if rms(velRes) < minRMS(segment,2)
                            
                            % Redefine index, minimal RMS and relevant dataset
                            index(segment,  2)          = i;
                            minRMS(segment, 2)          = rms(velRes);
            ...
            if (index(segment, 2)+samples(segment)) ~= length(IMU.C2(relevantDataset(segment,2)).Top(idx_freq).vel_x) && all(vel_x(index(segment,2)+1:index(segment,2)+samples(segment)))
                velRes_next     = rms(sqrt(IMU.C2(relevantDataset(segment, 2)).Top(idx_freq).vel_x((index(segment,2)+1):(index(segment,2)+samples(segment)))+IMU.C2(relevantDataset(segment, 2)).Top(idx_freq).vel_z(index(segment,2)+1:(index(segment,2)+samples(segment)))));
                assert(minRMS(segment, 2) < velRes_next,     'The RMS is not a minimum.')

            elseif index(segment,2) ~= 1
                velRes_previous = rms(sqrt(IMU.C2(relevantDataset(segment, 2)).Top(idx_freq).vel_x((index(segment,2)-1):(index(segment,2)+samples(segment)-2))+IMU.C2(relevantDataset(segment, 2)).Top(idx_freq).vel_z(index(segment,2)-1:(index(segment,2)+samples(segment)-2))));
                assert(minRMS(segment, 2) < velRes_previous, 'The RMS is not a minimum.')
            end
